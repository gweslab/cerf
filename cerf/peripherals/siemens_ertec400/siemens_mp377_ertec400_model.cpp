...
void SiemensMp377Ertec400Model::SaveState(StateWriter& writer) const {
    writer.Write("words_count", static_cast<uint64_t>(words_.size()));
    for (const auto& entry : words_) {
        writer.Write("word_address", entry.first);
        writer.Write("word_value", entry.second);
    }
    writer.Write("mdio_control", mdio_control_);
    writer.Write("last_mdio_register", last_mdio_register_);
    for (uint32_t index = 0; index < 32u; ++index) {
        writer.Write("mdio_phy_regs", mdio_phy_regs_[index]);
        writer.Write("mdio_phy_written", static_cast<uint32_t>(mdio_phy_written_[index] != 0u));
    }
    writer.Write("switch_control", switch_control_);
    writer.Write("switch_status", switch_status_);
    writer.Write("free_running_counter_10ns", FreeRunningCounter10ns());
    writer.Write("primary_readback", primary_readback_);
    writer.Write("secondary_readback", secondary_readback_);
    writer.Write("primary_readback_pending", static_cast<uint32_t>(primary_readback_pending_));
    writer.Write("secondary_readback_pending", static_cast<uint32_t>(secondary_readback_pending_));
    writer.Write("irt_start_time", irt_start_time_);
    writer.Write("switch_port_control", switch_port_control_);
    writer.Write("cyclic_counter_low", cyclic_counter_low_);
    writer.Write("cyclic_counter_high", cyclic_counter_high_);
    writer.Write("cyclic_counter_increment", cyclic_counter_increment_);
    writer.Write("cyclic_control", cyclic_control_);
    writer.Write("multicast_port_mask", multicast_port_mask_);
}

void SiemensMp377Ertec400Model::RestoreState(StateReader& reader) {
    uint64_t count = 0;
    reader.Read("words_count", count);
    words_.clear();
    for (uint64_t index = 0; index < count; ++index) {
        uint32_t key = 0;
        uint32_t value = 0;
        reader.Read("word_address", key);
        reader.Read("word_value", value);
        words_[key] = value;
    }
    reader.Read("mdio_control", mdio_control_);
    reader.Read("last_mdio_register", last_mdio_register_);
    for (uint32_t index = 0; index < 32u; ++index) {
        uint32_t written = 0;
        reader.Read("mdio_phy_regs", mdio_phy_regs_[index]);
        reader.Read("mdio_phy_written", written);
        mdio_phy_written_[index] = static_cast<uint8_t>(written != 0u);
    }
    reader.Read("switch_control", switch_control_);
    reader.Read("switch_status", switch_status_);
    reader.Read("free_running_counter_10ns", counter_base_10ns_);
    counter_epoch_us_ = HostSteadyMicros();
    reader.Read("primary_readback", primary_readback_);
    reader.Read("secondary_readback", secondary_readback_);
    uint32_t primary_pending = 0;
    uint32_t secondary_pending = 0;
    reader.Read("primary_readback_pending", primary_pending);
    reader.Read("secondary_readback_pending", secondary_pending);
    primary_readback_pending_ = primary_pending != 0u;
    secondary_readback_pending_ = secondary_pending != 0u;
    reader.Read("irt_start_time", irt_start_time_);
    reader.Read("switch_port_control", switch_port_control_);
    reader.Read("cyclic_counter_low", cyclic_counter_low_);
    reader.Read("cyclic_counter_high", cyclic_counter_high_);
    reader.Read("cyclic_counter_increment", cyclic_counter_increment_);
    reader.Read("cyclic_control", cyclic_control_);
    reader.Read("multicast_port_mask", multicast_port_mask_);
}
