...
public:
    ...
    void SaveState(StateWriter& writer) const {
        ...
        writer.Write("control", control_);
        writer.Write("tx_slot0", tx_slot0_);
        writer.Write("tx_slot1", tx_slot1_);
        writer.Write("tx_slot2", tx_slot2_);
        writer.Write("tx_slot3", tx_slot3_);
        writer.Write("tx_slot4", tx_slot4_);
        writer.Write("rx_slot0", rx_slot0_);
        writer.Write("rx_slot1", rx_slot1_);
        writer.Write("rx_slot2", rx_slot2_);
        writer.Write("rx_slot3", rx_slot3_);
        writer.Write("rx_slot4", rx_slot4_);
        writer.Write("drop_count", drop_count_);
        writer.Write("irq_pending", irq_pending_);
        writer.Write("codec_ready", codec_ready_);
        writer.Write("bclk_running", bclk_running_);
        writer.Write("tx_dirty_mask", tx_dirty_mask_);
        writer.Write("frame_phase", frame_phase_);
        writer.Write("codec_ready_frames", codec_ready_frames_);
        writer.Write("codec_read_pending", codec_read_pending_);
        writer.Write("codec_read_register", codec_read_register_);
        writer.Write("codec_read_data", codec_read_data_);
    }
    void RestoreState(StateReader& reader) {
        std::lock_guard<std::recursive_mutex> lock(state_mutex_);
        RestoreCodecState(reader);
        reader.Read("control", control_);
        reader.Read("tx_slot0", tx_slot0_);
        reader.Read("tx_slot1", tx_slot1_);
        reader.Read("tx_slot2", tx_slot2_);
        reader.Read("tx_slot3", tx_slot3_);
        reader.Read("tx_slot4", tx_slot4_);
        reader.Read("rx_slot0", rx_slot0_);
        reader.Read("rx_slot1", rx_slot1_);
        reader.Read("rx_slot2", rx_slot2_);
        reader.Read("rx_slot3", rx_slot3_);
        reader.Read("rx_slot4", rx_slot4_);
        reader.Read("drop_count", drop_count_);
        reader.Read("irq_pending", irq_pending_);
        reader.Read("codec_ready", codec_ready_);
        reader.Read("bclk_running", bclk_running_);
        reader.Read("tx_dirty_mask", tx_dirty_mask_);
        reader.Read("frame_phase", frame_phase_);
        reader.Read("codec_ready_frames", codec_ready_frames_);
        reader.Read("codec_read_pending", codec_read_pending_);
        reader.Read("codec_read_register", codec_read_register_);
        reader.Read("codec_read_data", codec_read_data_);
    }
