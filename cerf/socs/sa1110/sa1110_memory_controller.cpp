...
class Sa1110MemoryController : public Peripheral {
public:
...
uint8_t Sa1110MemoryController::ReadByte(uint32_t addr) {
...
uint32_t Sa1110MemoryController::ReadWord(uint32_t addr) {
    const uint32_t off = addr - MmioBase();
    uint32_t index;
...
void Sa1110MemoryController::WriteByte(uint32_t addr, uint8_t value) {
    const uint32_t off   = addr - MmioBase();
    const uint32_t base  = off & ~0x3u;
    const uint32_t shift = (off & 0x3u) * 8;
    uint32_t index;
...
void Sa1110MemoryController::WriteWord(uint32_t addr, uint32_t value) {
    const uint32_t off = addr - MmioBase();
    uint32_t index;
...
REGISTER_SERVICE(Sa1110MemoryController);
